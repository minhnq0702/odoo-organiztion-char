...
QUnit.test("primitives.common.family.loopRoots -  Loop sub-trees by size from smallest to largest.", function (assert) {
	...
	var isValid = JSON.stringify(members) == JSON.stringify(expected) && family.validate();
	assert.ok(isValid, "Function loopRoots returned following items: " + JSON.stringify(expected) + (isValid ? "" : " but returned " + JSON.stringify(families)));

	assert.ok(family.countParents(2) == 2, "Item 2 has 2 parent nodes.");
	assert.ok(family.countChildren(12) == 1, "Item 12 has 1 child node.");
	assert.ok(family.countParents(1) == 0, "Item 1 has no parents.");
	assert.ok(family.countChildren(9) == 0, "Item 9 has no children.");
	assert.ok(family.firstChild(12) == "11", "Item 12 has first child 11.");
	assert.ok(family.firstParent(13) == "14", "Item 13 has first parent 14.");
});
