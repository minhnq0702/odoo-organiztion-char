...
QUnit.test("primitives.common.Vector - 2D vector defined with 2 2D points", function (assert) {
	...
	assert.ok(p1.distanceTo(p2) == 10, "Distance between two points.");

	assert.ok(v1.relateTo(v2) == primitives.common.VectorRelationType.Collinear, "Vectors are colinear.");
	assert.ok(v1.relateTo(v3) == primitives.common.VectorRelationType.Opposite, "Vectors are opposite.");
	assert.ok(v1.relateTo(v4) == primitives.common.VectorRelationType.None, "Vectors have intersection.");
	assert.ok(v5.relateTo(v6) == primitives.common.VectorRelationType.None, "Vectors have intersection.");
	assert.ok(v6.relateTo(v7) == primitives.common.VectorRelationType.Null, "One of the vectors is null.");
